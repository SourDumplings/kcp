转自：https://luyuhuang.tech/2020/12/09/kcp.html
...
![](https://luyuhuang.tech/assets/images/kcp_1.svg)
...
- `resendts` 4 字节: 重传时间戳. 超过这个时间表示该报文超时, 需要重传
- `rto` 4 字节: 该报文的 RTO, 下次超时等待时间
- `fastack` 4 字节: ACK 失序次数. 也就是 KCP Readme 中所说的 "跳过" 次数.
- `xmit` 4 字节: 该报文传输的次数.
...
![](https://luyuhuang.tech/assets/images/kcp_2.svg)
...
![](https://luyuhuang.tech/assets/images/kcp_3.svg)
...
### 4.2.2. 接收
接收窗口结构如下图所示. 这里灰色节点表示顺序正确, 已经移动到 `rcv_queue` 中的报文. 注意下图表示的是 `rcv_queue` 和 `rcv_buf` 的结合体, 灰色节点都在 `rcv_queue` 中, 其余 `rcv_nxt` 及其右边的部分为 `rcv_buf`. 滑动窗口的起始位置为 `rcv_queue` 的队首, 大小为 `rcv_wnd`.

![](https://luyuhuang.tech/assets/images/kcp_4.svg)
...
![](https://luyuhuang.tech/assets/images/kcp_5.svg)
...
![](https://luyuhuang.tech/assets/images/kcp_6.svg)
...
![](https://luyuhuang.tech/assets/images/kcp_7.svg)
...
然后当 incr 累计增加的值超过一个 mss 时, cwnd 增加 1, 保持 incr 与 cwnd 之间的倍数关系. 因为 $incr = cwnd \cdot mss$
, 所以有
